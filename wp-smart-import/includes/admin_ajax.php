	...
	class wpsiAjaxController {
		...
		public static function render_xml_elements_for_filtring(DOMElement $el, $path ='', $lvl = 0) {	
			if ("" != $path) { 
				...
				echo '<option value="'. esc_attr( $path ).'['. esc_attr( self::$option_paths[$path] ) .']">'.esc_attr( $path ).'['. esc_attr( self::$option_paths[$path] ).']</option>';
			}
			else $path = $el->nodeName;		
					
			foreach ($el->attributes as $attr) {
				echo '<option value="'. esc_attr( $path ) .'['. esc_attr( self::$option_paths[$path] ) .']'. '/@' . esc_attr( $attr->nodeName ) .'">'. esc_attr( $path ) .'['. esc_attr( self::$option_paths[$path] ) .']'. '@' . esc_attr( $attr->nodeName ). '</option>';
			}
			if ($el->hasChildNodes()) {
				foreach ($el->childNodes as $child) {
					if ($child instanceof DOMElement) 
		...
		public static function render_xml_element(DOMElement $el, $shorten = false, $path = '/', $ind = 1, $lvl = 0)
		{
			...
			if ($el->hasChildNodes()) {
				...
				echo '<div class="xml-tag opening">&lt;<span class="xml-tag-name">' . esc_attr( $el->nodeName ) . '</span>'; self::render_xml_attributes($el, $path . '/'); echo '&gt;</div>';
				...
				echo '<div class="xml-tag closing">&lt;/<span class="xml-tag-name">' . esc_attr( $el->nodeName ) . '</span>&gt;</div>';
			} else {
				echo '<div class="xml-tag opening empty">&lt;<span class="xml-tag-name">' . esc_attr( $el->nodeName ) . '</span>'; self::render_xml_attributes($el); echo '/&gt;</div>';
			}
			echo '</div>';
		}
		...
		protected static function render_xml_text($text, $shorten = false, $is_render_collapsed = false, $is_cdata = false)
		{
			...
			echo '<div class="xml-content textonly' . ($is_short ? ' short' : '') . ($is_render_collapsed ? ' collapsed' : '') . '">' . esc_attr( $text ) . esc_attr( $more ). '</div>';
		}
		...
		{
			foreach ($el->attributes as $attr) {
				echo ' <span class="xml-attr" title="' . esc_attr( $path ) . '@' . esc_attr( $attr->nodeName ) . '"><span class="xml-attr-name">' . esc_attr( $attr->nodeName ) . '</span>=<span class="xml-attr-value">"'. esc_attr($attr->value) . '"</span></span>';
			}
		}	
		...
		{ ?>
			<ul id="menu-<?php echo esc_attr( str_replace('/', '-', $path ) ); ?>" class="ui-helper-hidden">
			    ...
			    <li data-command="action1" title="<?php echo esc_attr($path . '[@'. $attr->nodeName .' = "' . esc_attr($attr->value) . '"]'); ?>">
			    	<a href="#"><?php echo esc_attr( $path ) . '[@'. esc_attr( $attr->nodeName ) .' = "' . esc_attr($attr->value) . '"]'; ?></a>
			    </li>
			    <li data-command="action2" title="<?php echo esc_attr($path . '[@'. $attr->nodeName .'[contains(.,"' . esc_attr($attr->value) . '")]]'); ?>">
			    	<a href="#"><?php echo esc_attr( $path ) . '[@'. esc_attr( $attr->nodeName ) .'[contains(.,"' . esc_attr($attr->value) . '")]]'; ?></a>
			    </li>
				<?php endforeach; ?>
				<?php 
				$altNode = null;
	   ...
	   /*--------------------END ---------------------*/
             	...
             	<thead>
                 	<tr>
                 	 	<td colspan="3" class="node-title">
                 	 		<h3 style="color:#e14d43;margin-top: 0;">
                 	 			<?php echo esc_attr( "Element :".$selected_node ); ?>
                 	 		</h3><hr>		
                 	 	</td>
                 	</tr>
                 	<?php if ($total_node > 1): ?>
                    <tr>
                        <td> 
                            ...
                            <?php echo $node_num == 1 ? "button-disabled":''; ?>" data-val="<?php echo esc_attr($node_num-1) ?>"
                        ...
                        <td>
                            ...
                            <input type="hidden" name="node_num" value="<?php echo esc_attr($node_num ); ?>" />
                           	<span class="total-element"><?php echo esc_attr( $total_node ); ?></span>
                        </td>
                        <td>
		...
		static public function wpsi_insertImport($postdata, $task) {
			...
			if (empty($id)) { return $res; }
		    	...
		    	$res['status'] = "success";
		    	$res['msg'] = "1 Import ".esc_attr( $status )." <a href='". esc_url("$admin_url?page=$pages[1]&action=edit&id=$id&_nonce=$nonce") ."'>Edit</a> ";
		    	$_SESSION['res'] = $res;
		    	if ($task == "save") {
		    		wpSmartImport::wpsi_redirect(array('page' => $pages[1]));
	    		} else {
	    			wpSmartImport::wpsi_redirect(array('page' => $pages[1], 'action' => 'update', 'id' => $id, '_nonce' => $nonce ));
	    		}
		    }
		}

		static function wpsi_runImport() {
			$nonce = isset($_REQUEST['_nonce']) ? sanitize_text_field(wp_unslash($_REQUEST['_nonce'])) : "";
			if ( ! wp_verify_nonce( $nonce, 'wpsi_nonce' ) ) {
				wp_die( esc_html__( 'Security check failed. Please try again.', 'wp-smart-import' ) );
			}
			...
			$post_data = wpsi_helper::TrimArray( unserialize( $data->post_data, array( 'allowed_classes' => array( 'wpsi_hpostdata' ) ) ) );
	    	$pdata = wpSmartImportCommon::parse_post_data($post_data, $batchs);
			$batches = array_keys($request['batches']);
	    	$end_key = end( $batches );
	    	$data = self::insert_post(wpsi_helper::recursive_sanitize_text_field( wp_unslash( $pdata ) ), $import_id, $batchs);
		...
		static function insert_post($pdata, $import_id, $batchs) {
			$wpSmartImportCommon = new wpSmartImportCommon;
			$wpsiQuery = new wpSmartImportQuery;
			global $session;
			$res = array('status' => 'error', 'msg' => 'Error Try Again !');
			...
			if (!empty($pdata)) {
				...
				foreach ($pdata as $indx => $pvalue) {
					...
					$new_post = array(
					  ...
					  'post_content'   => html_entity_decode($pvalue['post_des']),
		...
		static function wpsi_images_preview() {
			$wpSmartImportCommon = new wpSmartImportCommon;
			$nonce = isset($_REQUEST['_nonce']) ? sanitize_text_field( wp_unslash( $_REQUEST['_nonce'] ) ) : "";
			if ( ! wp_verify_nonce( $nonce, 'wpsi_nonce' ) ) {
				wp_die( esc_html__( 'Security check failed. Please try again.', 'wp-smart-import' ) );
		...
		static function insert_term() {
			...
			wpSmartImportCommon::verify_ajax($nonce);
			$wpSmartImportCommon = new wpSmartImportCommon;
			$_POST = wpsi_helper::recursive_sanitize_text_field( wp_unslash( $_POST ) );
		...
		static function get_total_batch_for_import() {
			$nonce = isset($_REQUEST['_nonce']) ? sanitize_text_field(wp_unslash($_REQUEST['_nonce'])) : "";
			if ( ! wp_verify_nonce( $nonce, 'wpsi_nonce' ) ) {
				wp_die( esc_html__( 'Security check failed. Please try again.', 'wp-smart-import' ) );
			}
		...
		static function get_total_batch_for_file() {
			$nonce = isset($_REQUEST['_nonce']) ? sanitize_text_field(wp_unslash($_REQUEST['_nonce'])) : "";
			if ( ! wp_verify_nonce( $nonce, 'wpsi_nonce' ) ) {
				wp_die( esc_html__( 'Security check failed. Please try again.', 'wp-smart-import' ) );
			}
		...
		static function manage_import_files() {
			$request = wpsi_helper::recursive_sanitize_text_field( wp_unslash( $_POST ) );
			$nonce = isset($_REQUEST['_nonce']) ? sanitize_text_field( wp_unslash( $_REQUEST['_nonce'] ) ) : "";
			if ( ! wp_verify_nonce( $nonce, 'wpsi_nonce' ) ) {
				wp_die( esc_html__( 'Security check failed. Please try again.', 'wp-smart-import' ) );
		...
		static function manage_imports() {
			$request = wpsi_helper::recursive_sanitize_text_field( wp_unslash( $_POST ) );
			$nonce = isset($request['_nonce']) ? sanitize_text_field( wp_unslash( $request['_nonce'] ) ) : "";
			if ( ! wp_verify_nonce( $nonce, 'wpsi_nonce' ) ) {
				wp_die( esc_html__( 'Security check failed. Please try again.', 'wp-smart-import' ) );
			}
			wpSmartImportCommon::verify_ajax($nonce);
		...
		static function delete_post($posts, $hard_delete = false) {
			$wpsiQuery = new wpSmartImportQuery;
			$post_counter = 0;
