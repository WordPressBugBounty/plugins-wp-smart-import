jQuery( document ).ready(function( $ ){
	...
	function prepare_upload(event) {
		var file = event.target.files;
	  	...
	  	sendData.append("_nonce", wpsi_file_ajax.nonce); 
    	...
    	$.ajax({
			...
			error: function(xhr, textStatus, errorThrown) {
				...
				if (xhr.responseJSON && xhr.responseJSON.message) {
					...
					errorMessage += ' Status: ' + textStatus + ', Error: ' + errorThrown;
				}
	...
	$('.xml_element_list .wpsi-root-element').on("click", function() {
	...
	$( ".file-input" ).on("keyup change", function(e) {
	...
	$("#file_name_check").on("click", function(e){
		e.preventDefault();
	          	...
	          	success: function(data, textStatus, jqXHR) {
	          		if (data.status == 'success') {
		          		...
		          		if (data.proccessing == 'run') { 
		          			...
		          			Progress.hide();
		          			$('#run_import').prop( 'disabled', false ).html('Run Import');
	...
	var ids = [];
	var  post_type ='' ;
	$('#delete_imports').on("click", function() {
