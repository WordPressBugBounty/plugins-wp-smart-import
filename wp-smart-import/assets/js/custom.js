jQuery( document ).ready(function( $ ){
    ...
    $.fn.xml = function (opt) {
        ...
        if (action.dragable) { // drag & drop
            ...
            $drag.css({
                ...
                "borderRadius": '0',
                "zIndex":'99'
            }).appendTo(document.body).on( "mousedown", function (e) {
                if (_dbl) return;
            ...
            $('#title, #content, .widefat, input[name^=custom_name], textarea[name^=custom_value], input[name^=featured_image], input[name^=unique_key]').on('focus', insertxpath );
    ...
    $('.show_hide_radio').on('change',function( e ){
    ...
    /*Button Tabs*/
    $('.button-tabPannel .wpsi-button').on("click", function(){
    ...
    $('#useFile').on('change',function(e) { // will use filename inputbox  hide/show  
        if ($(this).is(":checked")) {
